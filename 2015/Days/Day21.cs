...
{
    private readonly List<(int cost, int damage, int armor)> weaponList = [(8, 4, 0), (10, 5, 0), (25, 6, 0), (40, 7, 0), (74, 8, 0)];
    private readonly List<(int cost, int damage, int armor)> armorList = [(13, 0, 1), (31, 0, 2), (53, 0, 3), (75, 0, 4), (102, 0, 5)];
    private readonly List<(int cost, int damage, int armor)> ringList = [(25, 1, 0), (50, 2, 0), (100, 3, 0), (20, 0, 1), (40, 0, 2), (80, 0, 3)];
    private readonly List<(int cost, int damage, int armor)> empty = [(0, 0, 0)];
    ...
    public string SolveFirst()
    {
        ...
        {
            foreach (var armor in empty.Concat(armorList))
            {
                foreach (var ring in empty.Concat(ringList).Concat(ringList.Pairs().Select(x => (x.First.cost + x.Second.cost, x.First.damage + x.Second.damage, x.First.armor + x.Second.armor))))
                {
    ...
    public string SolveSecond()
    {
        ...
        foreach (var weapon in weaponList)
        {
            foreach (var armor in empty.Concat(armorList))
            {
                foreach (var ring in empty.Concat(ringList).Concat(ringList.Pairs().Select(x =>
